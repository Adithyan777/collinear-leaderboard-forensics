...
def resume_plan(state, shards):
    ...
    A shard is done only once mark_done() ran, i.e. its rows committed. The
    interrupted shard's transaction never committed (sqlite rolled it back),
    so it is simply still pending -- score it and everything after it exactly
    once. Nothing already committed is ever re-scored.
    """
    state["in_progress"] = None
    save(state)
    return [s for s in shards if s not in state["completed"]]
