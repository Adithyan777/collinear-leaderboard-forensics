"""[EXCLUDED FROM GRADING - blind-reachability rule] mutant_18: violates SPEC 6.1 -- judges validity against the passed weights keys instead of the canonical 12-label taxonomy (SPEC: validity is always judged against the taxonomy, regardless of weights).
Killing input: weights {"billing": 1, "widget": 1} + prediction "widget" for gold billing: ref treats "widget" as INVALID (FN only, widget fp=0); mutant treats it as valid -> widget fp=1
Generated from ref_scorer.py by dev/make_mutants.py.
EXCLUDED from the graded set: zero blind kills across every blind suite
evaluated against it; retained ungraded as a documented difficulty knob.
See tests/mutants/MUTANTS.md."""
...
def score_model(gold, predictions, weights):
    ...
    for cat in weights:
        ...
        per_category[cat] = {"tp": c["tp"], "fp": c["fp"], "fn": c["fn"], "f1": f1}
