"""mutant_17: violates SPEC 6.2 -- rounds intermediate per-category F1s to 4dp BEFORE the weighted aggregation (SPEC: all intermediate values stay unrounded); reported f1 stays unrounded, unlike mutant_13.
Killing input: billing f1=2/3 and refunds f1=0.5 with equal weights: true overall 7/12 -> 0.5833; mutant aggregates (0.6667+0.5)/2 = 0.58335 -> 0.5834
...
def score_model(gold, predictions, weights):
    ...
    num = sum(weights[c] * round(per_category[c]["f1"], 4) for c in weights if c in gold_present)
