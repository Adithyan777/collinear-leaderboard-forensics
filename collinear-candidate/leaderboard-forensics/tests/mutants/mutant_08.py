"""mutant_08: violates SPEC 5.1 -- uses recall instead of F1.
Killing input: any category with FP>0 or asymmetric FP/FN scores differently
...
def score_model(gold, predictions, weights):
    ...
    for cat in weights:
        ...
        f1 = (c["tp"] / (c["tp"] + c["fn"])) if (c["tp"] + c["fn"]) else 0.0
