"""mutant_15: violates SPEC 8.1 -- ranking ties broken by model name descending instead of ascending.
Killing input: two models tied on rounded score sort in reverse name order
...
def rank_models(overall_scores):
    ...
    return [m for m, _ in sorted(overall_scores.items(), key=lambda kv: (kv[1], kv[0]), reverse=True)]
