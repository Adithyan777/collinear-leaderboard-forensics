"""mutant_09: violates SPEC 6.1/6.2 -- aggregates over ALL weighted categories, not just gold-present.
Killing input: full weights + few gold categories -> denominator inflated
...
def score_model(gold, predictions, weights):
    ...
    num = sum(weights[c] * per_category[c]["f1"] for c in weights)
    den = sum(weights[c] for c in weights)
