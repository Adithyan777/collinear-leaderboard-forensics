"""mutant_11: violates SPEC 3.3 -- dedup keeps first VALID occurrence instead of first occurrence.
Killing input: duplicate pair invalid-then-valid: ref scores the invalid first row (FN); mutant rescues the valid second
...
def score_model(gold, predictions, weights):
    ...
    for row in predictions:
        ...
        if eid in seen:
            continue
        f = normalize_label(row["label"])
        if f not in TAXONOMY_WEIGHTS:
            continue  # unusable row; a later duplicate may be valid
        seen[eid] = row["label"]
