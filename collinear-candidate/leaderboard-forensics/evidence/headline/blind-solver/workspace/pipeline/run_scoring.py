...
from pipeline.data_io import connect, read_jsonl, replace_shard_rows, reset_db
...
def run(resume=False, log=print):
    ...
    if resume:
        ...
        # Reset the checkpoint before the DB: if we die between the two, a
        # stale "completed" list must not make --resume skip shards whose rows
        # were just dropped.
    ...
    for shard in plan:
        ...
        replace_shard_rows(conn, shard, rows)
